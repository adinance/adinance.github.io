...
if(isset($_POST['btnSubmit'])=='Login'){
    ...
    if(
            $username=='ticamporn.s'  // บี
            || $username=='auchara.j' // หมีพู
            || $username=='janejira'
           ) {
        ...
        // echo '<script language="JavaScript">window.location="main.php?menu=employee_list&pages=employee_list&company_id='.$row['company_id'].'&cmo_employee_status=1";</script>';

    }

    if($username!='orawan.nak'){
        if($bind 
        || $username=='natthapong.k' 
        || $username=='nantiya.s'  
        || $username=='sakdinan.s'
        || $username=='chayapha.p'
        || $username=='sawitree.c'
